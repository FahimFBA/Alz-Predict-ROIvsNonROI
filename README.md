> [!IMPORTANT]
> **This repository is archived and is no longer actively maintained.** No further updates, bug fixes, or new experiments will be made. The code and results are preserved here for reference and reproducibility of the published paper. Please do not open issues expecting resolution; the repository is kept public solely as a research artifact.
...
> [!WARNING]
> **Dataset availability notice:** The preprocessed ADNI and OASIS datasets that were previously hosted publicly on Kaggle have been permanently deleted. Both ADNI and OASIS contacted us directly and requested the immediate removal of the data from all public platforms, and we complied accordingly. Additionally, the CSV files containing subject IDs were also removed following a separate request received via email from ADNI. We do not retain copies of these datasets in any form. To access the original data, researchers must apply directly through the official [ADNI](https://adni.loni.usc.edu/) and [OASIS](https://www.oasis-brains.org/) portals.
...
> [!WARNING]
> **Missing notebooks — experiments 3, 4, 9, and 10:** The notebooks for these four experiments were accidentally lost and are no longer recoverable. Because the experiments were extremely computationally expensive (large-scale 3D MRI training runs), we did not rerun them after the loss. The reported metrics were recorded at the time of the original runs and remain accurate as published in the paper.
...
|       3        | OASIS - AD vs MCI| 98%  | 98.3%            | Notebook accidentally lost — not rerun due to high compute cost | Resnet18 with CBAM |
|       4        | OASIS (6 ROIs) - AD vs MCI | 98.33% | 97.78% | Notebook accidentally lost — not rerun due to high compute cost | Resnet18 with CBAM |
...
|       9        | ADNI - CN vs MCI | 87.27% | 89.5%          | Notebook accidentally lost — not rerun due to high compute cost | Resnet18 with CBAM |
|       10       | ADNI (6 ROIs) - CN vs MCI | 88.2% | 91.03% | Notebook accidentally lost — not rerun due to high compute cost | Resnet18 with CBAM |
|       11       | OASIS - CN vs MCI | 98.05% | 98.89%        | [View](https://nbviewer.org/github/FahimFBA/Alz-Predict-ROIvsNonROI/blob/main/Notebooks/OASIS/Complete%20Region%20of%20Human%20Brain/Upgrade%20Filter%20allreg%20OASIS%20MCI%20vs%20CN/Upgraded_MCIvsCN_Filter_allreg_oasis.ipynb) -- [Notebook](Notebooks/OASIS/Complete%20Region%20of%20Human%20Brain/Upgrade%20Filter%20allreg%20OASIS%20MCI%20vs%20CN/Upgraded_MCIvsCN_Filter_allreg_oasis.ipynb) | Resnet18 with CBAM |
...
This repository is archived and no further maintenance is planned. If you have any questions regarding the research, the methodology, or the published paper, please contact the team lead:
